      ...
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-6xl w-[90vw] max-h-[80vh] overflow-auto relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {pages.map((text, idx) => (
            <div
              key={idx}
              className={`border rounded p-2 text-xs h-32 overflow-y-auto cursor-pointer relative ${searchTerm && text.toLowerCase().includes(searchTerm.toLowerCase()) ? 'ring-2 ring-blue-400' : ''} ${bookmarks.includes(idx) ? 'bg-yellow-100' : ''}`}
              onClick={() => onJump(idx)}
            >
              <div className="absolute top-1 right-2 text-blue-400 text-xs">
                {idx + 1}
              </div>
              <div className="absolute top-1 left-2 text-yellow-400">
